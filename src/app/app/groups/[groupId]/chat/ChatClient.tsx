...
// Sync server props -> local state (para router.refresh y evitar estados �pegados�)
React.useEffect(() => {
...
React.useEffect(() => {
  ...
  // hard reset del composer (evita que se quede texto “pegado”)
  const hardResetComposer = React.useCallback(() => {
    setDraft("");
    setReplyTo(null);
    setEditing(null);
    setPendingFile(null);
  ...
  const resizeInput = React.useCallback(() => {
    ...
    if (!el) return;
    el.style.height = "0px";
    const h = Math.min(el.scrollHeight, 112); // max ~7 líneas
  ...
  // Mobile keyboard: cuando cambia el viewport, mantén el chat abajo si estabas abajo
  React.useEffect(() => {
...
const markRead = React.useCallback(
    async (iso: string) => {
      ...
      if (error) {
        // ignoramos: el chat sigue funcionando en modo básico
      }
    },
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          // si estamos abajo, bajamos y marcamos leído
  ...
  const loadOlder = React.useCallback(async () => {
    ...
    try {
      ...
      setError(e?.message ? String(e.message) : "No se pudo cargar m�s mensajes");
    } finally {
  ...
  // ---------- Send / edit
    ...
    try {
      ...
      // OPTIMISTIC (aparece instantáneo)
      ...
      // No hacemos select() aquí para evitar fallos por permisos/RLS.
      // El optimistic ya está en pantalla y router.refresh() sincroniza.
...
// ---------- Actions
  ...
  const typingLabel = React.useMemo(() => {
    ...
    return `Varias personas est�n escribiendo...`;
  }, [typingUsers]);
    ...
    <div className="mt-4">
      ...
      <div className="relative flex h-[calc(100dvh-220px)] min-h-[70vh] flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        >
          ...
          {sorted.length === 0 && (
            <div className="mt-10 text-center text-sm text-slate-500">A�n no hay mensajes. �Di hola! 👋</div>
          )}
          ...
          <div className="mt-3 space-y-2">
            {sorted.map((m, idx) => {
              ...
              const name = mine ? "T�" : m.profiles?.full_name || "Miembro";
                        ...
                        {replied && !isDeleted && (
                          ...
                          >
                            <div className="truncate font-semibold">
                              {replied.user_id === myId ? "T�" : replied.profiles?.full_name || "Miembro"}
                            </div>
...
function AttachmentView({
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // si no existe bucket o política, intentamos publicUrl (por si el bucket es público)
        try {
  ...
  if (!url) {
    return (
      <div className={cls("mb-2 rounded-xl p-2 text-xs", mine ? "bg-white/10" : "bg-slate-50")}>Adjunto�</div>
    );
  }
